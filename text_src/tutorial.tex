...
\subsection{Sample the bound state and define the binding restraint}\label{step:equilibrium}
    ...
    \item \textbf{Define the Distance-to-Bound Configuration (DBC) Coordinate}
        \begin{enumerate}[label=\alph*., ref=\theenumi.\alph*]
            ...
            \end{itemize}
            \item \label{step:symmetry}\textbf{Identify equivalent atoms:} %\jh{shorten this}
             \begin{itemize}
    ...
    \begin{enumerate}[label=\arabic*.]
        ...
        \begin{enumerate}[label=\alph*., ref=\theenumi.\alph*]
            ...
            \begin{itemize}
                ...
                \item Set the atoms of group 2 to \textInput{dummyAtom (x0, y0, z0)} where x0, y0, and z0 are the coordinates of the geometric center of the ligand. Your editor should look like the figure below. Note the inclusion of commas in the \textInput{dummyAtom} statement.\\  \includegraphics[width=0.8\textwidth]{Figures/CVDBscreenshots/COM_restraint.png} \label{fig:COMtext}
            ...
            \begin{itemize}
                ...
                \item Using the \menu{Atoms from selection text:} tool in the left panel, enter \textInput{resname PHEN and noh} and click \button{Insert [Enter]}. \label{step:setAtoms}
        ...
        \begin{enumerate}[label=\alph*., ref=\theenumi.\alph*]
            ...
            \begin{itemize}
                ...
                \item Modify the bias to match the following parameters (see Appendix~\ref{app:RFEP}):\\
                { \ttfamily
                \begin{tabular}{l l}
        ...
        \begin{enumerate}[label=\alph*., ref=\theenumi.\alph*]
            ...
            \begin{itemize}
                ...
                \includegraphics[width=0.5\linewidth]{Figures/sample_TI_traj.png}
            \end{itemize}
            ...
            \begin{itemize}
                ...
                \item Run all the cells in that section. Sample outputs and details regarding interpretation can be found in Appendix~\ref{app:RFEP}
...
\subsection{Decoupling from solvent}
...
\textbf{\textInput{IDWS}} (interleaved double-wide sampling) %\grace{specify literal parameter} 
...
\begin{equation}\label{eq:dUwalldlambda}
    ...
    \end{cases} 
\end{equation}
% To improve numerical behavior near $\lambda = 0$ (when the force is completely released), we use a non-linear dependence:

% where the exponent $\alpha$ is defined by the Colvars parameter \texttt{targetForceExponent}, and set to 6 in this tutorial.
...
%The relationship is not strictly equivalent, because the unoccupied ensemble may include configurations in which the ligand is interacting with the protein but not occupying the site. However, the discrepancy is expected to vanish for small $m$ (when the ligand is very dilute and will rarely collide with the protein) and for large $m$ (when many non-decoupled ligands will be available for interact with the protein surface). Thus we assume here that such configurations make a negligible contribution to the difference in free energy between the occupied and the unoccupied states. 
...
%This degree of precision is not merited in this toy example because it would require either much longer simulations or contrived start conditions; either of which would increase the computation time considerably without improving user competence in the technique.
...
\end{figure}
%Multimodal DBC Distributions may lead the user to set a DBC restraint that is much too wide. This will cause convergence issues later on.\\
\textbf{Causes:}\\
...
\textbf{DBC Debugging Checklist:}%\ezry{point the user back to the relevant steps. Or maybe this should be part of the "define the DBC" step above.}
...
Double check the choice of wall position and ensure that it is correct in all Colvar config files.\\
        ...
        %% Move to Appendix?
        % Please see the comments in the provided NAMD FEP configuration file , the appendix on FEP parameters \ref{app:FEPparameters}, or the \href{https://www.ks.uiuc.edu/Research/namd/2.14/ug/node62.html}{NAMD User's Guide} for more detailed explanations. The important settings for this tutorial are:
        %     \begin{itemize}
        %         \item \textInput{wrapAll off} is a precaution against miscalculation of collective variable parameters due to over-small periodic cells.
        %         \item Colvars settings - these induce the DBC restraint to prevent unbinding.
        %         \item Alchemy settings - the lambda schedule, the length of each lambda window, and relevant input and output files.
        %         \item \textInput{runFEP  0.0 1.0 \$dLambda \$numSteps  true}
        %         - this is what does the actual FEP calculations. The final 'true' turns on IDWS (Interleaved Double-Wide Sampling) which effectively halves the amount of compute time required to complete a FEP cycle as described in \ref{app:IDWS}. %\jesse{link to appendix; need to explain/boast about IDWS as it is one of the components that makes SAFEP so great?}
        %     \end{itemize}




% \section{HOLDING AREA FOR UNUSED TEXT}
% \subsection{}
% A longer simulation of the ligand-protein system serves two purposes: 1) it gives us the opportunity to validate the parameters and initial conditions of the system and 2) it allows us to sample the bound ensemble in the absence of external restraints which is necessary for parameterizing the restraint we will use during FEP. Three numerical computations are then carried out on the ligand: FEP from bound to gas-phase, FEP from bulk to gas-phase, and thermodynamic integration from the DBC-restrained to center-of-mass-restrained ensembles. Lastly, the center-of-mass restraint is addressed analytically and the parts are summed to get the overall free energy of binding as shown in Equation \ref{eq:overalldG}.

% \subsection{DBC/restraint problems}
% Issues with the system's restraints can cause convergence issues without interfering with the sensibility of the trajectory itself. It is difficult, for example, to discern a restraint issue during the site decoupling by examining the trajectory alone. For this reason, we recommend both examining the trajectory \textit{and} using the diagnostic tools built into the Colvars Dashboard. Specifically, the timeline tools available for both the colvar itself and the energy of any bias. \ezry{add screenshot showing sample timelines}


% \subsection{Why Phenol/Lysozyme}
...
% While phenol does not bind significantly to the apolar cavity in wild-type or L99 mutant T4 lysozyme\cite{Deng2006,Wei2002,Morton1995,Graves2005}, it does bind the site created by the L99A/M102H double mutation, with an experimental binding affinity $ \Delta G_{bind}^{o, exp}$ of -5.44~kcal/mol \cite{Merski2013}, to be compared with the result we find for Eq.~(\ref{eq:overalldG}).

% \subsection{Ligand binding free energy: Alchemlyb library}
%  An alternative to the FEP calculation is the alchemlyb library that recently supports the NAMD's IDWS feature \cite{chodera2016simple, shirts2008statistically}, providing the BAR and the MBAR estimators. A jupyter notebook documented by \href{https://github.com/BranniganLab/safep}{Dr. Grace Brannigan's Lab}, /[path]/Jupyter-notebook/BAR\_NAMD\_alchemlyb.ipynb, conveniently sets up a broad series of built-in functions and tools of the alchemlyb library in a straightforward framework, including parsing the fepout file, FEP calculation, as well as qualitatively comparing the free energy results in different plot types. Launch a jupyter notebook in the directory of \texttt{BAR\_NAMD\_alchemlyb.ipynb} file:

%  For the forward and reverse sampling, the alchemlyb library uses the Bennett acceptance ratio (BAR) estimator to calculate the difference free energy for each window and estimate the overall free energy at $\lambda$=1 by ensemble averaging over all $\lambda$-windows. Correspondingly, the evolution of the phenol binding free energy relative to $\lambda$ is obtained from parsing the \texttt{AFEP2-02.fepout} with the jupyter notebook and illustrated in Figure %(\ref{fig:AFEP-decoupling1}).
%  According to Figure %(\ref{fig:AFEP-decoupling1})
%  and the screenshot of the printed data in the notebook represented in Figure %(\ref{fig:ipynb-cell4}),
%  the phenol decoupling affinity from the binding site is estimated at about (14.65$\pm$0.38) kcal/mol.



% \subsection{Extracting the ligand pdb file}
% \label{app:extractpdb}
 
%     \begin{enumerate}[label*=\arabic*.]
%         \item Load your reference pdb. 
%         \item Then, in the tkconsole run:
%         \begin{lstlisting}[language=tcl]
% set ligand [atomselect top "resname PHEN"]
% ::TopoTools::selections2mol $ligand
% animate write pdb PHEN.pdb
% animate write psf PHEN.psf
%         \end{lstlisting}
