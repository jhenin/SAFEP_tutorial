...
Follow the steps below for a basic calculation of the binding affinity of phenol to lysozyme. More detail on the rationale for each step, as well as alternative options, can be found in the hyperlinked sections.  
...
\end{verbatim} 
Note that you need to do this for both instances of the keyword (one in the atom group, and one in the RMSD block).
In the Colvar dashboard main menu, click on "Save colvars" at the top, change to the directory and save the "DBC ligand RMSD" into a file with the ".colvars" extension.
\item In a text editor, create a separate Colvars configuration file "histogram.colvars" for the histogram feature, with the following content:
\begin{verbatim}
histogram {
  colvars DBC
  outputFile DBC.dat
...
\begin{verbatim}
namd2  /Supp-Files/DBC-Unbiased/equ.namd
\end{verbatim}
...
\begin{verbatim}
/Supp-Files/Python-scripts/python 1-Plot-histogram.py
\end{verbatim}
...
\begin{verbatim}
namd2 /Supp-Files/AFEP-Bound-Decoupling/equ.namd
\end{verbatim}
After equilibration, then perform the production run:
\begin{verbatim}
namd2 /Supp-Files/AFEP-Bound-Decoupling/run.namd
\end{verbatim}
...
\begin{verbatim}
namd2 \Supp-Files\RFEP\run.namd 
\end{verbatim}
...
%\grace{do they need to plot it? we could just have the awk script above sum it?}\Mina{Brilliant Suggestion! I think these two lines work. Each dA/d{$\lambda$} needs to be multiplied by d{$\lambda$} before making sum.}
\end{enumerate}
\item  {\bf \hyperref[sec:11]{Decoupling from solvent}}
\begin{enumerate}
\item Equilibrate the ligand in solvent by running
\begin{verbatim}
namd2 \Supp-Files\AFEP-Hydration\equ.namd
\end{verbatim}
\item Decouple ligand from solvent
\begin{verbatim}
namd2 \Supp-Files\AFEP-Hydration\run.namd
\end{verbatim}
\item Parse the " .fepout" file using the ipython notebook BAR\_NAMD\_alchemlyb.ipynb, as in Step 2c.  
%I could not upload the .fepout as it is too large.
\grace{which notebook? Do we provide two copies of the alchemlyb notebook?}
...
% TODO: rendering where the protein structure is clearer.
}
\label{fig:assembly}
\end{figure}
...
Look into \texttt{/Supp-File/AFEP-Bound-Decoupling/equ.namd} configuration file.
...
The fep file format is required to introduce outgoing and incoming atoms for the AFEP evaluation. In this case, the phenol is annihilated in the binding site and the rest of the system is remained without alterations, so beta-column for the phenol should be set to -1 in the fep file format. You can Create the fep file by using the \texttt{fep.tcl} script enclosed as supplementary information.
...
\end{verbatim}
The accurate estimation of free energy differences ($\Delta F$) provided by BAR or SOS methods is highly dependent on a sufficiently large number of samples. Hence, the \texttt{alchOutFreq} should be assigned small enough to improve the evaluation of the free energy differences by decreasing error bars.
\begin{verbatim}
...
\end{verbatim}
The alchemical FEP sampling is performed by 20 windows with equal intervals i.e. {$\Delta\lambda$=0.05}, each window is run for 500000 steps interleaved with 500 equilibration steps.
...
\subsection{Result assessment}
The AFEP simulation output data file is analyzed with VMD to assess the free energy result and the accuracy of the sampling strategy. Before going through the detail of FEP analysis via VMD, it is necessary to post-process the output file. Although you take advantage of a reliable and a cheap simulation using interleaved double-wide sampling, it is required to extract the forward and the backward data out of the fepout data file. For this purpose, a python script is used by executing the subsequent command.
\begin{verbatim}
.\python  lib_alch_deinterleave_idws.py  AFEP2-02.fepout
\end{verbatim}
As a result of running the python script, two output data files, bwd and fwd fepout files, are generated attributed to backward and forward free energy values. VMD 1.9.4, currently supported on Linux and macOS operating systems, is needed to proceed with the free energy estimation, using the ParseFEP plugin \cite{Liu2012}. In the parameter section of the ParseFEP plugin, we set the \texttt{Gram-Charlier order} to 0 and enable pictorial representation by checking the \texttt{disp} option. The forward and the backward outfep files are chosen as FEP and FEP (backward) output files and \texttt{run FEP Parsing}, employing the \texttt{BAR estimator}. The ParseFEP graphical interface provides free energy estimation combining forward and backward results with the help of the \texttt{SOS} or the \texttt{BAR estimator}. In Figure (\ref{fig:AFEP-decoupling1}), you can find phenol free energy results, decoupling from the binding site environment.

\begin{figure}[th!]
\centering
\includegraphics[width=\linewidth]{AFEP-decoupling-sum}
\caption{The {$\lambda$}-evolution of the forward and the backward phenol decoupling free energy, from the lysozyme binding site, is evaluated using the ParseFEP plugin in VMD.}
\label{fig:AFEP-decoupling1}
\end{figure}
